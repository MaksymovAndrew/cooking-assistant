...
- Comments are a last resort: the default is NO comment; write one only for a genuinely non-obvious "why", and keep it to one short line. Style: plain `//` with a single space and a lowercase first letter (acronyms / proper nouns like JWT, SQL, URL, Express keep their case, e.g. `// JWT login`). The old `//?` / `//!` prefixes were removed - don't reintroduce them.
