...
Registration requires a unique `email` alongside `login` (both enforced by DB unique constraints, with distinct `409` error codes). Every domain error can carry a stable `code` alongside its message (`ERROR_CODES`/`ERROR_MESSAGES` in [backend/src/constants/errorMessages.ts](backend/src/constants/errorMessages.ts)), surfaced by `errorHandler` as `{ error, code }` on 4xx responses, so the frontend shows the exact right copy per cause instead of guessing from the HTTP status. Password reset (`POST /forgot-password` + `POST /reset-password`) and email verification (`PUT /email`, `POST /resend-verification-email`, `POST /confirm-email`) reuse the session JWT's signing mechanism as short-lived purpose-scoped tokens (`TokenService.generatePurposeToken`/`verifyPurposeToken`, a `purpose` claim); password-reset tokens are additionally bound to a fingerprint of the password hash at issue time, so a link stops verifying the moment it's used once.
