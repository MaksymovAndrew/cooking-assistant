...
Pages are organized by domain folder under [frontend/src/pages/](frontend/src/pages/) (`auth`, `home`, `recipes`, `user-recipes`, `menu`, `user-menu`, `person-ingredients`, `statistics`, `profile`, `settings`, `not-found`). Reusable UI lives in [frontend/src/components/](frontend/src/components/), grouped by domain plus `layout/`, `ui/`, and `icons/`. The stats page renders its charts with `recharts` (lazy-loaded page chunk); PDF export was removed in 3.3. Recipe types are reference data behind a read-only API - there is no recipe-types page or component folder (the CRUD page was removed in 1.40).
...
- `react-router` GHSA-qwww-vcr4-c8h2 (unstable RSC/React Server Components APIs, not reachable from this plain client-side SPA on `createBrowserRouter`) no longer shows up in `npm audit` as of 4.2 either. **`react-router-dom` still has no 8.x release** (7.18.2 is latest, tracking `react-router@7.18.2`) even though `react-router` itself reached 8.3.0 - re-check this pairing before assuming it's resolved for good.
