...
**To add or change an ingredient:** the catalog is data, not code - 739 entries in [backend/src/scripts/catalog/catalogData.json](backend/src/scripts/catalog/catalogData.json), keyed by `slug`. Add an object (`slug`, `category`, `nameEn`/`nameRu`/`nameUk`/`namePl`, `unit`, `allergens`, `daysToExpire`, `seasonality`, `storageCondition`, `caloriesPerUnit`), then run `npm --prefix backend run catalog:locales` to regenerate the frontend `catalog.json` locale files, then `npm run seed`. Seeding upserts (`ON CONFLICT (slug) DO UPDATE`), so it re-runs on every deploy and keeps the DB in sync with the JSON - editing an existing entry propagates, it isn't insert-once. Commit the JSON, the regenerated locales, and nothing else. Do NOT write a migration for this - ingredients are rows, not schema.
