...
- All three `package.json` files pin `brace-expansion` per major line (`brace-expansion@1`/`@2`/`@3`) to the backported releases that fix GHSA-mh99-v99m-4gvg. **`npm audit` still reports these as vulnerable and that is a false positive**: the advisory's affected range is written `<=5.0.7`, which numerically covers the fixed `1.1.18`/`2.1.4`/`3.0.6` backports. Do NOT "fix" it with a blanket `"brace-expansion": "^5"` override - 5.x switched to a named export and old `minimatch@3` does `require("brace-expansion")` expecting the function itself, which crashes ESLint with `expand is not a function`. It reaches the tree only through eslint/jest/testcontainers (dev-only; the backend prod image installs `--omit=dev` and the frontend ships static files), so there is no production exposure. Drop the overrides once the advisory ranges are corrected upstream.
- The remaining `npm audit` finding on the frontend is `react-router` GHSA-qwww-vcr4-c8h2, which only affects the unstable RSC (React Server Components) APIs - this app is a plain client-side SPA on `createBrowserRouter`, so it is not reachable. It is patched in `react-router@8.3.0`, but **`react-router-dom` has no 8.x release yet** (7.18.2 is latest) and npm's suggested "fix" is a downgrade to 7.11.0. Bump it when react-router-dom 8 ships; do not add a Dependabot ignore, which would block that real fix.
