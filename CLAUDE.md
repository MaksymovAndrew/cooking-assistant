...
3. Connects to the server over SSH and runs `/srv/bin/deploy.sh` ([deploy/deploy.sh](deploy/deploy.sh)): pull images, run migrations + seed as a one-shot compose container, start the stack, wait for **both** containers to report healthy - and restore the previous `IMAGE_TAG` if either never does. Gating on the backend alone was enough while the frontend was static files; a render process can start and still serve nothing.
...
- `user.routes` -> register, login, logout, current-user profile, password reset, email verification
...
**Error codes are the contract; display text lives in the catalog.** Every client-facing error is an `AppError` built from a code in [backend/src/constants/errorCodes.ts](backend/src/constants/errorCodes.ts) (`ERROR_CODES`, namespaced by domain: `recipe/not_found`, `auth/session_expired`) - never from a message string. `AppError` holds no display text: `errorHandler` resolves it at the HTTP edge as `detail ?? translateError(code)`, which is the one place a request locale can be read once a second language exists. All server-side copy lives in JSON catalogs under [backend/src/i18n/locales/](backend/src/i18n/locales/) (`errors.json` keyed by code, `messages.json` for `{ message }` success bodies via `translateMessage`, `email.json` via `getEmailCopy`) behind [backend/src/i18n/translate.ts](backend/src/i18n/translate.ts), whose `satisfies Catalog` makes a key missing from any locale a compile error. A new error = one `ERROR_CODES` entry + one `errors.json` line + the same entry in `frontend/src/constants/errorCodes.ts` (a backend test fails if that mirror drifts in either direction) + its user-facing copy under `apiErrors` in `frontend/src/i18n/locales/en/common.json` (a frontend test fails if a code has none). The frontend renders error text from that copy by code - `getApiErrorMessage` falls back to the server's English `error` only for a code the build doesn't know. A new locale = one `locales/<locale>/` folder + one `CATALOGS` entry. Misconfiguration or programming failures (missing JWT secret, a route without `req.user`) are plain `Error`s, not `AppError`s - every 5xx answers `server_error` and never leaks its message.
...
- **TypeScript and ESLint majors are pinned on purpose** (see the `ignore` block in [.github/dependabot.yml](.github/dependabot.yml)) - re-check these before assuming they are still stuck, the picture moves:
    - **TypeScript 7**: hard-blocked. `typescript-eslint` peers on `typescript >=4.8.4 <6.1.0` in both its stable (8.69.0) and canary (8.69.1-alpha.\*) lines, and it is the parser behind every type-aware lint rule. TS 6.0.3 is the ceiling until that peer range moves.
